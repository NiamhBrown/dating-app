...
export const FeedPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      getUsers(token)
        .then((data) => {
          const current_user = data.users.filter(
          ...
          const other_users = data.users.filter((user) =>
          ...
          if (current_user.lookingFor.proficiencyLevel == "") {
            ...
            setUsers(
              other_users.filter(
                (x) =>
                  x.proficiencyLevel == current_user.lookingFor.proficiencyLevel
              )
            );
          }
  ...
  const incriment = () => {
    setPosition((prevPosition) => {
      ...
      const newPosition =
        prevPosition < users.length - 1 ? prevPosition + 1 : 0;
  ...
  return (
    <>
      <div>
        ...
        {users.length == 0 && <p>No new users</p>}
      </div>
    </>
  );
};
